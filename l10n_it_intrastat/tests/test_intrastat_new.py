...
TEST_ACCOUNT_MOVE_LINE = {
    ...
    'z0bug.move_ZI_9_1': {
        'move_id': 'z0bug.move_ZI_9',
        'product_id': 'z0bug.product_product_23',
        'name': 'Servizi ad alto valore aggiunto',
        'quantity': 150,
        'account_id': 'external.4101',
        'price_unit': 0.89,
        'move_line_tax_ids': 'z0bug.tax_a41a',
        'rc': True,
    },
    'z0bug.move_ZI_9_2': {
        'move_id': 'z0bug.move_ZI_9',
...
# Test Result
# Items are move lines: key is (move_id, statement_section)
TRES_MOVE = {
    '%s~%s' % ('z0bug.move_Z0_4', 'sale_s1'): {
        'intrastat_code': '84439910',
    },
    '%s~%s' % ('z0bug.move_Z0_4', 'sale_s3'): {
        'intrastat_code': '582929',
    },
    '%s~%s' % ('z0bug.move_ZI_9', 'purchase_s1'): {
        'intrastat_code': '84439910',
    },
    '%s~%s' % ('z0bug.move_ZI_9', 'purchase_s3'): {
        'intrastat_code': '582929',
    },
}
# Item are move intrastat lines: key is (move_id, statement_section)
TRES_INTRASTAT = {
    '%s~%s' % ('z0bug.move_Z0_4', 'sale_s1'): {
        'intrastat_code': '84439910',
...
class TestAccountMove(common.TransactionCase):
    ...
    def __test_intrastat_moves(self):
        model = 'account.move'
        model_child = 'account.move.line'
        for xref in TEST_ACCOUNT_MOVE:
            _logger.info(
                "🎺 Testing %s[%s]" % (model, xref)
            )
            vals = self.get_values(
                model,
                TEST_ACCOUNT_MOVE[xref])
            inv = self.model_make(model, vals, xref)

            for xref_child in TEST_ACCOUNT_MOVE_LINE.values():
                if xref_child['move_id'] == xref:
                    vals = self.get_values(model_child, xref_child)
                    vals['move_id'] = inv.id
                    self.model_make(model_child, vals, False)
            # inv.compute_taxes()
            # inv.action_move_open()
            inv.action_post()
        ...
        # There are 4 move lines with intrastat:
        # sale-product / sale-service / purchase-product / purchase-service
        move_checked = {}
        for hash_code, test_intra in TRES_MOVE.items():
            (move_id, statement_section) = hash_code.split('~', 1)
            if move_id not in move_checked:
                move_checked[move_id] = 0
            # intrastat = TRES_INTRASTAT[hash]
            move = self.env_ref(move_id)
            for line in move.move_line_ids:
                intrastat_code = line.product_id.intrastat_code_id.name
                if intrastat_code != test_intra['intrastat_code']:
                    continue
                # Count line checked
                move_checked[move_id] += 1
                if line.product_id.type == 'service':
                    section = '%s_s3' % move.journal_id.type
                else:
                    section = '%s_s1' % move.journal_id.type
                self.assertEqual(
                    section, statement_section,
                    msg='Wrong intrastat section!'
                )
        for move, ctr in move_checked.items():
            self.assertEqual(
                ctr, 2,
                msg='Not all move lines has been checked!'
            )
        ...
        for hash_code, test_intra in TRES_INTRASTAT.items():
            ...
            intrastat = self.env['account.move.intrastat'].search(
                [('move_id', '=', move.id),
