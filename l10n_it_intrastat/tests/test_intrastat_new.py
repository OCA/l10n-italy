...
TRES_INVOICE = {
    "%s~%s"
    % ("z0bug.invoice_Z0_4", "sale_s1"): {
        "intrastat_code": "84439910",
    },
    "%s~%s"
    % ("z0bug.invoice_Z0_4", "sale_s3"): {
        "intrastat_code": "582929",
    },
    "%s~%s"
    % ("z0bug.invoice_ZI_9", "purchase_s1"): {
        "intrastat_code": "84439910",
    },
    "%s~%s"
    % ("z0bug.invoice_ZI_9", "purchase_s3"): {
        "intrastat_code": "582929",
    },
}
...
class TestAccountMove(common.TransactionCase):
    ...
    def test_intrastat_invoices(self):
        ...
        for xref in TEST_ACCOUNT_INVOICE:
            ...
            self.assertEqual(inv.intrastat, True, msg="Flag intrastat not set!")
        ...
        for hash_code, test_intra in TRES_INVOICE.items():
            ...
            for line in invoice.invoice_line_ids:
                ...
                self.assertEqual(
                    section, statement_section, msg="Wrong intrastat section!"
                )
        for _, ctr in invoice_checked.items():
            self.assertEqual(ctr, 2, msg="Not all invoice lines has been checked!")
