...
class AccountMoveLine(models.Model):
    ...
    def _prepare_intrastat_line_country_payment(self, res):
        ...
        if self.move_id.move_type in ('out_invoice', 'out_refund'):
            country_payment_id = \
                self.move_id.company_id.partner_id.country_id
            ...
            country_payment_id = \
                self.move_id.partner_id.country_id
        res.update({
    ...
    def _prepare_intrastat_line_payment(self, res):
        ...
        if self.move_id.payment_term_id \
                and self.move_id.payment_term_id.intrastat_code:
    ...
    def _prepare_intrastat_line_province_dest(self, company_id, res):
        ...
        if self.move_id.move_type in ('out_invoice', 'out_refund'):
            province_destination_id = \
                self.move_id.partner_id.state_id
        elif self.move_id.move_type in ('in_invoice', 'in_refund'):
            province_destination_id = \
    ...
    def _prepare_intrastat_line_amount(self, res):
        ...
        amount_euro = move_currency._convert(
            amount_currency,
            company_currency,
    ...
    def _prepare_intrastat_line_transaction_b(self, company_id, res):
        self.ensure_one()
        if self.move_id.move_type in ('receivable', 'receivable_refund'):
            res.update({
                'transaction_nature_b_id':
                    company_id.intrastat_sale_transaction_nature_b_id.id
            })
        elif self.move_id.move_type in ('payable', 'payable_refund'):
            res.update({
                'transaction_nature_b_id':
...
class AccountMove(models.Model):
    ...
    intrastat_line_ids = fields.One2many(
        comodel_name='account.move.intrastat',
    ...
    def action_move_create(self):
        for move in self:
            if not move.intrastat_line_ids and move.intrastat:
                move.compute_intrastat_lines()
        ...
        for move in self:
            if move.intrastat:
                ...
                total_amount = sum(
                    l.amount_currency for l in move.intrastat_line_ids)
                subtotal = abs(move.amount_untaxed - excluded_amount)
            ...
            for line in inv.move_line_ids:
                ...
                if i_code_id in i_line_by_code:
                    ...
                    intra_line['statement_section'] = \
                        self.env['account.move.intrastat'] \
                            .compute_statement_section(i_code_type, inv.move_type)
...
class AccountMoveIntrastat(models.Model):
    _name = 'account.move.intrastat'
    _description = "Intrastat Line"

    def name_get(self):
        res = []
        for l in self:
            res.append((l.id, '%s' % l.move_id.name))
        return res
    ...
    def _compute_amount_euro(self):
        for line in self:
            ...
            move_currency = line.move_id.currency_id
            if move_currency:
                line.amount_euro = move_currency._convert(
    ...
    def _compute_partner_data(self):
        for line in self:
            line.country_partner_id = line.move_id.partner_id.country_id

    @api.depends('move_id.ref',
                 'move_id.name', 'move_id.invoice_date')
    def _compute_move_ref(self):
        for line in self:
            if line.move_id.move_type in ['in_invoice', 'in_refund']:
                if not line.move_id.ref:
                    continue
                line.move_name = line.move_id.ref \
    ...
    @api.depends('move_id.move_type', 'intrastat_code_type')
    def _compute_statement_section(self):
        """
        Compute where the move intrastat data will be computed.
        This field is used to show the right values to fill in
        """
        move_type_ctx = self.env.context.get('move_type')
        intrastat_code_type_ctx = self.env.context.get('intrastat_code_type')
        for statement in self:
            move_type = move_type_ctx or statement.move_id.move_type
            intrastat_code_type = \
                intrastat_code_type_ctx or statement.intrastat_code_type
            statement.statement_section = self.compute_statement_section(
                intrastat_code_type, move_type)

    @api.model
    def compute_statement_section(self, intrastat_code_type, move_type):
        section = False
        # Purchase
        if move_type in ('in_invoice', 'in_refund'):
            if intrastat_code_type == 'good':
                ...
                if move_type == 'in_invoice':
                    ...
                    section = 'purchase_s4'
        # Sale
        elif move_type in ('out_move', 'out_refund'):
            if intrastat_code_type == 'good':
    ...
    def change_weight_kg(self):
        if self.move_id.company_id.intrastat_additional_unit_from == \
                'weight':
