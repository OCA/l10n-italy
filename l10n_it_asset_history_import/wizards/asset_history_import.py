...
class AssetHistoryImport(models.TransientModel):
    ...
    def download_template_file(self):
        ...
        """
        assets = (
            self.env["asset.depreciation.line"]
            .search(
                [
    ...
    def check_before_import(self):
        if not self.file:
            raise ValidationError(_("No file to import!"))
        ...
        if filename.split(".")[-1].lower() not in ("xls", "xlsx"):
            raise ValidationError(
                _(
                    "Cannot determine file extension. Please check its"
    ...
    def convert_to_asset_vals(self, asset_dict, workbook, sheet):
        ...
        curr = self.env["res.currency"].search([("name", "=", asset_dict[curr_col])])
        if not curr:
            raise ValidationError(
                _(
                    "Could not find currency for name '{}'.\n"
                    "Is it in its ISO 4217 format?"
                )
            )
        supplier_vat_col = get_header_by_model_and_field("res.partner", "vat").col
        ...
        if supplier_vat:
            ...
            if not partner:
                raise ValidationError(
                    _("Could not find Supplier with VAT {}".format(supplier_vat))
                )

        supplier_doc_ref_col = get_header_by_model_and_field(
            "asset.asset", "supplier_ref"
        ).col
    ...
    def convert_to_dep_vals(self, dep_dict, workbook, sheet):
        ...
        written into the DB (via a `create()` or `write()`
        """
        vals = convert_via_headers(dep_dict, "asset.depreciation", workbook, sheet)
        ...
        dep_mode = self.get_obj_by_import_code("asset.depreciation.mode", mode_code)
        dep_type = self.get_obj_by_import_code("asset.depreciation.type", type_code)
    ...
    def import_assets_from_data(self, file_data, workbook, sheet):
        ...
        for data in grouped_data.values():
            ...
            asset_vals = self.convert_to_asset_vals(asset_data, workbook, sheet)
            ...
            if asset_code in asset_code_cache:
                ...
                if not asset:
                    ...
                    raise ValidationError(
                        _(
                            "Cannot determine the assets to update, found"
                            " multiple assets with same code `{}`."
                        ).format(asset_code)
                    )
                asset_code_cache[asset_code] = asset

            dep_vals = self.convert_to_dep_vals(data["dep_data"], workbook, sheet)
            dep = dep_obj.search(
                [
                    ("asset_id", "=", asset.id),
            ...
            lines_vals = self.convert_to_lines_vals(data["lines_data"], workbook, sheet)
    ...
    def parse_file(self):
        ...
        if not file_headers:
            raise ValidationError(_("File headers must be set in the first row."))
        if len(HEADERS) != len(file_headers):
            raise ValidationError(
                _(
                    "Headers number mismatch: aborting import."
