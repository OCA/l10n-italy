...
class riba_distinta(orm.Model):
    ...
    _columns = {
        ...
        'config': fields.many2one('riba.configurazione', 'Configuration',
                                  select=True, required=True, readonly=True, states={
        ...
        for distinta in self.browse(cr, uid, ids, context=context):
            if distinta.state not in ('draft',  'cancel'):
                raise orm.except_orm(
                    _('Error'), _('Distinta %s is in state %s. You can only delete documents in state draft or canceled') % (distinta.name, distinta.state))
    ...
    def confirm(self, cr, uid, ids, context=None):
        ...
        for distinta in self.browse(cr, uid, ids, context=context):
            line_pool.confirm(
                cr, uid, [line.id for line in distinta.line_ids], context=context)
        return True
