...
class riba_file_export(orm.TransientModel):
    ...
    def _creaFile(self, intestazione, ricevute_bancarie):
        accumulatore = self._RecordIB(intestazione[0], intestazione[
                                      1], intestazione[4], intestazione[5], intestazione[6])
    ...
    def act_getfile(self, cr, uid, ids, context=None):
        ...
        if not order_obj.config.company_id.partner_id.vat and not order_obj.config.company_id.partner_id.fiscalcode:
            raise orm.except_orm(
                'Error', _('No VAT or Fiscalcode specified for: ') + name_company)
        array_testata = [
            credit_sia,
               credit_abi,
               credit_cab,
               ...
               order_obj.config.company_id.partner_id.vat and order_obj.config.company_id.partner_id.vat[
                   2:] or order_obj.config.company_id.partner_id.fiscalcode,
        ]
        arrayRiba = []
        for line in order_obj.line_ids:
            ...
            if not debit_bank.iban:
                raise orm.except_orm(
                    'Error', _('No IBAN specified for ') + line.partner_id.name)
            ...
            if not line.partner_id.vat and not line.partner_id.fiscalcode:
                raise orm.except_orm(
                    'Error', _('No VAT or Fiscal code specified for ') + line.partner_id.name)
            if not (debit_bank.bank and debit_bank.bank.name or debit_bank.bank_name):
                raise orm.except_orm(
                    'Error', _('No debit_bank specified for ') + line.partner_id.name)
            Riba = [
                line.sequence,
                        due_date,
                        line.amount,
                        ...
                        line.invoice_number,
                        # datetime.datetime.strptime(line.distinta_id.date_created,
