...
class RibaEffectResult(models.TransientModel):
    ...
    _description = \
        "Wizard which let the user to choose if a effect is paid or unsolved"
    ...
    def _onchange_account_id_expense_amount(self, new_account, new_amount):
        """
            function  which update the wizard view when
        ...
        if self.id:
            ...
            for WIline in self.accout_move_detail_table:
                if WIline.type_of_line == 'bank':
                    note_ids.append(
                        (1, WIline.id, {'account_id': new_account}))
                else:
                    note_ids.append((1, WIline.id, {}))
            value.update(accout_move_detail_table=note_ids)
            return {'value': value}
    ...
    riba_distinta_line_ids = fields.Many2many(
        'riba.distinta.line', string="riba distinta lines linked",
        readonly=True,
    ...
    def action_payment_executed(self):
        ...
        totale_importo = self.create_link_to_Linked_Wiz_RiBa(partner_INFO,
                                                             'paid')
    ...
    def create_link_to_Linked_Wiz_RiBa(self, partner_INFO, situation='paid'):
        """
            Create in the support class for this wizard
        ...
        if situation == 'unsolved':
            ...
            data = self.get_dictionary_to_create(
                False, self.bank_expense_account_id.id,
                self.expense_amount, 0.00, 'expense')
            self.env['wizard.linked_ribaeffectresult'].create(data)

        data = self.get_dictionary_to_create(False, self.bank_account_id.id,
                                             0.00, totale, 'bank')
        self.env['wizard.linked_ribaeffectresult'].create(data)
    ...
    def organize_partner_info(self, list_to_organize):
        """
            organize a len = 2 list of tuple
    ...
    def skip(self):
        """
        function to change the state of each distinta line in
    ...
    def create_new_movement(self):
        """
        Firstable all the riba distinta line will be checked, to
        verify their state is in accredited, and all the fields which are
        mandatory are not NULL.
        Second a new account move will be created: the new
        ...
        movement
        Third, all the correct lines in this account move will be
        reconciled with the correct lines of the acceptance move
        In the end, all the invoice involved will be linked to this
        movement
    ...
    def check_in_function(self):
        """
        This function check if all the mandatory fields are
        valorized
        """
        if (
...
# ================================================ #
class WizardlinkedRibaeffectresult(models.TransientModel):
    _name = 'wizard.linked_ribaeffectresult'
    _description = \
        "This class is used to support the wizard RibaEffectResult " \
        "in order to show the partner selected and their situation"
