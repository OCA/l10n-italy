...
class WizardGiornale(models.TransientModel):
        ...
        for wizard in self:
            if wizard.report_giornale and wizard.daterange:
                wizard.report_giornale_name = \
                    _("Account Central Journal - %s.pdf" % wizard.daterange.name)
    ...
    def _get_move_name(self, line):
        return " - ".join(filter(None, [line.move_id.ref, line.move_id.name]))

    def _get_account_name(self, line):
    ...
    def get_initial_balance_data_report_giornale(self):
        ...
        initial_balance_data = [[
            ...
            Paragraph(
                formatLang(self.env, self.progressive_debit2), style_number
            ),
            Paragraph(
    ...
    def get_final_tables_report_giornale(
        ...
        for line in self.env["account.move.line"].browse(move_line_ids):
            ...
            row = Paragraph(str(start_row), style_name)
            date = Paragraph(format_date(self.env, line.date), style_name)
            move_name = self._get_move_name(line)
            move = Paragraph(move_name, style_name)
            account_name = self._get_account_name(line)
            account = Paragraph(account_name, style_name)
    ...
    def create_report_giornale(self):
        ...
        if self.group_by_account:
            list_grupped_line = self.get_grupped_line_ids()
    ...
    def print_giornale(self):
        self.create_report_giornale()

        model_data_obj = self.env["ir.model.data"]
        view_rec = model_data_obj.get_object_reference(
            "l10n_it_central_journal", "wizard_giornale"
        )
    ...
    def print_giornale_final(self):
        end_row, end_debit, end_credit = self.create_report_giornale()
