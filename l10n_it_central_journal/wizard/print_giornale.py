...
#             'account.fiscalyear'].with_context(context=ctx).find()
#         return [fiscalyear_id]
    ...
    def _get_fiscalyear(self, cr, uid, context=None):
        ...
        fiscalyears = self.pool.get('account.fiscalyear').search(domain, limit=1)
    ...
    def _get_journal(self):
        ...
        #journal_ids = self.env['account.journal'].with_context(context=ctx).find()
        return journal_ids

    _name = "wizard.giornale"
  #  _rec_name = "type"

    date_move_line_from = fields.Date('From date',
                                      required=True)
    date_move_line_from_view = fields.Date('Last definitive date print')
    ...
    journal_ids = fields.Many2many(
        ...
        required=True)
    target_move = fields.Selection([('all', 'All'),('posted', 'Posted'), ('draft', 'Draft')], 'Target Move',default='all')
    ...
    def on_change_fiscalyear(self):
        ...
        if self.fiscalyear:
            ...
            date_start = datetime.strptime(self.fiscalyear.date_start,"%Y-%m-%d").date() 
            date_stop = datetime.strptime(self.fiscalyear.date_stop,"%Y-%m-%d").date() 
            if self.fiscalyear.date_last_print:
                date_last_print = datetime.strptime(self.fiscalyear.date_last_print,"%Y-%m-%d").date()
                self.date_move_line_from_view = date_last_print
    ...
    def print_giornale(self, cr, uid, ids, context=None):
        ...
        datas_form['date_move_line_from_view'] = wizard.date_move_line_from_view
    ...
    def print_giornale_final(self, cr, uid, ids, context=None):
        ...
        wizard = self.browse(cr, uid, ids[0], context=context)
        if wizard.date_move_line_from <=  wizard.date_move_line_from_view:
