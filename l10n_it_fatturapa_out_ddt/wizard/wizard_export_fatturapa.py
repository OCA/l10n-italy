...
class WizardExportFatturapa(models.TransientModel):
            ...
            for picking in invoice.picking_ids:
                for ddt in picking.ddt_ids:
                    if ddt.ddt_number:
                        ...
                        return res
            if invoice.transportation_method_id:
                res['include_ddt_data'] = 'dati_trasporto'
                return res
        return res
    ...
    def setDatiDDT(self, invoice, body):
        res = super(WizardExportFatturapa, self).setDatiDDT(
            invoice, body)
        p_line_obj = self.env['stock.picking.package.preparation.line']
        if self.include_ddt_data == 'dati_ddt':
            inv_lines_by_ddt = {}
            for line in invoice.invoice_line:
                p_line = p_line_obj.search([('move_id','=', line.move_line_ids.id)])
                if (
                    p_line and
                    p_line.package_preparation_id.ddt_number and
                    p_line.package_preparation_id.date
                ):
                    key = (
                        p_line.package_preparation_id.ddt_number,
                        p_line.package_preparation_id.date[:10]
                    )
            ...
            body.DatiGenerali.DatiTrasporto = DatiTrasportoType(
                ...
                TipoResa=invoice.incoterm.code or None
            )
