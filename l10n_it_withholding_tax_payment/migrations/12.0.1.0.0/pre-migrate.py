...
def _fix_date_field(cr):
    cr.execute(
        "SELECT id, date FROM withholding_tax_move_payment" " WHERE date IS NOT NULL;"
    )
    ...
    for record in records:
        ...
        try:
            ...
            _logger.error(
                "The row in table 'withholding_tax_move_payment'"
                " with ID #{} and 'date' field valued"
                " as '{}' will be set as NULL...".format(record_id, record_value)
            )

            cr.execute(
                "UPDATE withholding_tax_move_payment" " SET date = NULL WHERE id = %s;",
                (record_id,),
            )

    cr.execute(
        "ALTER TABLE withholding_tax_move_payment"
