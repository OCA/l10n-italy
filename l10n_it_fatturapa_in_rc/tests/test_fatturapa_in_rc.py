# -*- coding: utf-8 -*-
from openerp.addons.l10n_it_fatturapa_in.tests.fatturapa_common import (
    FatturapaCommon)
...
class TestInvoiceRC(FatturapaCommon):
    ...
    def test_00_xml_import(self):
        ...
        self.assertEqual(invoice.invoice_line[0].name, 'LA DESCRIZIONE')
        self.assertEqual(invoice.invoice_line[1].name, 'BANCALI')
        self.assertFalse(invoice.invoice_line[0].rc)
        self.assertTrue(invoice.invoice_line[1].rc)
        self.assertEqual(
            invoice.invoice_line[0].invoice_line_tax_id.name,
            '22% e-bill')
        self.assertEqual(
            invoice.invoice_line[1].invoice_line_tax_id.name,
        ...
        self.assertEqual(
            invoice.rc_self_invoice_id.invoice_line.
            invoice_line_tax_id.name,
            'Tax 22% Sales RC ITA'
        )
