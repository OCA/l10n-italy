...
class DichiarazioneIntento(models.Model):
    ...
    line_ids = fields.One2many(
        "dichiarazione.intento.line", "dichiarazione_id", string="Lines"
    )
    ...
    def create(self, values):
        ...
        if values.get("type", False) == "in":
            ...
            actual_limit_total = (
                sum([d.limit_amount for d in dichiarazioni]) + values["limit_amount"]
            )
    ...
    def _check_taxes_for_dichiarazione_intento(self):
        for dichiarazione in self:
            if (
                dichiarazione.taxes_ids
                and dichiarazione.fiscal_position_id
                and dichiarazione.fiscal_position_id.tax_ids
            ):
                taxes = [
                    t.tax_dest_id.id for t in dichiarazione.fiscal_position_id.tax_ids
                ]
...
class DichiarazioneIntentoLine(models.Model):
    ...
    dichiarazione_id = fields.Many2one("dichiarazione.intento", string="Declaration")
    taxes_ids = fields.Many2many("account.tax", string="Taxes")
    move_line_ids = fields.Many2many(
        "account.move.line", string="Move Lines", ondelete="cascade"
    )
