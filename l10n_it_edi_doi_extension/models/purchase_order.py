...
class PurchaseOrder(models.Model):
    ...
    l10n_it_edi_doi_not_yet_invoiced = fields.Monetary(
        ...
        store=True,
        readonly=True,
        help="Total under the Declaration of Intent of this"
        "document that can still be invoiced",
    )
    ...
    def _compute_l10n_it_edi_doi_id(self):
        for order in self:
            if (
                not order.l10n_it_edi_doi_use
                or order.state != "draft"
                and not order.l10n_it_edi_doi_id
            ):
    ...
    @api.depends(
        "l10n_it_edi_doi_id", "tax_totals", "order_line", "order_line.qty_invoiced"
    )
    ...
    def _l10n_it_edi_doi_check_configuration(self):
        """
        Raise a UserError in case the configuration of the purchase order is invalid.
        """
        errors = []
        for order in self:
            declaration = order.l10n_it_edi_doi_id
            ...
            doi_tax = order.company_id.l10n_it_edi_doi_bill_tax_id
            if not doi_tax:
                continue
    ...
    def _l10n_it_edi_doi_get_amount_not_yet_invoiced(
        ...
        """
        Consider purchase orders in self that use declaration of intent `declaration`.
        For each purchase order we compute the amount that is tax exempt due to the
        declaration of intent (line has special declaration of intent tax applied)
        but not yet invoiced. For each line of the PO we i.e. use the not yet invoiced
        quantity to compute this amount. The aforementioned quantity is computed from
        field `qty_invoiced` and parameter `additional_invoiced_qty`
        Return the sum of all these amounts on the POs.
        :param declaration:             We only consider purchase orders using
                                        Declaration of Intent `declaration`.
        :param additional_invoiced_qty: Dictionary (purchase order line id -> float)
                                        The float represents additional invoiced amount
                                        qty for the purchase order. This can be used
                                        to simulate posting an already linked invoice.
        """
        if not declaration:
            return 0
        ...
        tax = declaration.company_id.l10n_it_edi_doi_bill_tax_id
        if not tax:
            return 0
