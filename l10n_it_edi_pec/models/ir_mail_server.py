...
class IrMailServer(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None):
        # When ir.mail_server.connect() searches for a default SMTP server
        # (empty domain, ordered by sequence, limit 1), exclude PEC servers
        # so they are not used for regular emails.
        if domain == [] and order == "sequence" and limit == 1:
            domain = [("is_l10n_it_edi_pec", "=", False)]
