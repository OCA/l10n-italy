...
class TestPecSend(TestItEdiPecCommon):
    ...
    def test_ir_mail_server_search_excludes_pec(self):
        """PEC server must not be used as default SMTP server."""
        # Default SMTP lookup: search([], order='sequence', limit=1)
        servers = self.env["ir.mail_server"].search([], order="sequence", limit=1)
        for server in servers:
            self.assertFalse(
                server.is_l10n_it_edi_pec,
                "PEC server should not appear in default SMTP search",
            )
