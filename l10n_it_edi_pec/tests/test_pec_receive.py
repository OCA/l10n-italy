...
class TestPecReceive(TestItEdiPecCommon):
    def _process_pec_email(self, filename):
        ...
        # Temporarily allow cr.commit because core
        # _l10n_it_edi_write_send_state calls it
        original_commit = self.env.cr.commit
        self.env.cr.commit = lambda: None
        try:
            self.env["mail.thread"].with_context(
                fetchmail_server_id=self.pec_fetch_server.id
            ).message_process(False, incoming_mail)
        finally:
            self.env.cr.commit = original_commit
    ...
    @mute_logger(
        ...
        "odoo.addons.l10n_it_edi_pec.models.mail_thread",
    )
    def test_process_broken_xml_sends_notification(self):
        """Broken XML in incoming invoice sends error email to contacts."""
        outbound_mail_model = self.env["mail.mail"]
        error_mails_before = outbound_mail_model.search_count(
            [
                (
                    "recipient_ids",
                    "in",
        ...
        with mock.patch("odoo.addons.mail.models.fetchmail.IMAP4") as mock_imap:
            ...
            self.pec_fetch_server.fetch_mail()

        error_mails_after = outbound_mail_model.search_count(
            [
                (
                    "recipient_ids",
                    "in",
        ...
        self.assertGreater(error_mails_after, error_mails_before)
    ...
    def test_notification_type_from_filename(self):
        ...
        test_cases = [
            ...
            ("IT01234567890_12345_AT_001.xml", "AT"),
        ]
        for filename, expected_type in test_cases:
            result = MailThread._l10n_it_edi_pec_get_type_from_filename(filename)
            self.assertEqual(
                result,
                expected_type,
    ...
    def test_action_check_triggers_fetchmail(self):
        ...
        with mock.patch.object(type(self.pec_fetch_server), "fetch_mail") as mock_fetch:
