...
class res_company(orm.Model):
    ...
    _columns = {
        'withholding_payment_term_id':
            fields.many2one('account.payment.term',
                            'Withholding tax Payment Term',
                            help="The withholding tax will have to be paid"
                                 " within this term"),
        'withholding_account_id':
            fields.many2one('account.account',
                            'Withholding account',
                            help='Payable account used for amount'
                                 ' due to tax authority',
                            domain=[('type', '=', 'payable')]),
        'withholding_journal_id':
            fields.many2one('account.journal',
                            'Withholding journal',
                            help="Journal used for registration"
                                 " of witholding amounts to be paid"),
        'authority_partner_id': fields.many2one('res.partner',
            ...
            res['value'].update({
                'withholding_payment_term_id':
                    (company.withholding_payment_term_id and
                     company.withholding_payment_term_id.id or
                     False),
                'withholding_account_id': (company.withholding_account_id and
                                           company.withholding_account_id.id or
                                           False),
                'withholding_journal_id': (company.withholding_journal_id and
                                           company.withholding_journal_id.id or
                                           False),
                'authority_partner_id': (company.authority_partner_id and
                                         company.authority_partner_id.id or
                                         False),
            })
        else:
            res['value'].update({
...
class account_invoice(orm.Model):
    ...
    _columns = {
        'withholding_amount':
            fields.float('Withholding amount',
                         digits_compute=dp.get_precision('Account'),
                         readonly=True,
                         states={'draft': [('readonly', False)]}),
        'has_withholding':
            fields.boolean('With withholding tax',
                           readonly=True,
                           states={'draft': [('readonly', False)]}),
...
class account_voucher(orm.Model):
    ...
    def reconcile_withholding_move(self, cr, uid, invoice, wh_move,
                                   context=None):
        ...
        for wh_line in wh_move.line_id:
            if wh_line.account_id.type == 'payable' and \
        ...
        return line_pool.reconcile_partial(cr,
                                           uid,
                                           rec_ids,
                                           type='auto',
                                           context=context)
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context):
            amounts_by_invoice = super(account_voucher, self).\
                allocated_amounts_grouped_by_invoice(cr,
                                                     uid,
                                                     voucher,
                                                     context)
                ...
                if invoice.withholding_amount:
                    ...
                    if voucher.type != 'payment':
                        raise orm.except_orm(_('Error'), _(
                            'Can\'t handle withholding tax'
                            ' with voucher of type other than payment'))
                    if not invoice.company_id.withholding_account_id:
                        raise orm.except_orm(_('Error'), _(
                            'The company does not have'
                            ' an associated Withholding account'))
                    if not invoice.company_id.withholding_payment_term_id:
                        raise orm.except_orm(_('Error'), _(
                            'The company does not have'
                            ' an associated Withholding Payment Term'))
                    if not invoice.company_id.withholding_journal_id:
                        raise orm.except_orm(_('Error'), _(
                            'The company does not have'
                            ' an associated Withholding journal'))
                    if not invoice.company_id.authority_partner_id:
                        raise orm.except_orm(_('Error'), _(
                            'The company does not have'
                            ' an associated Tax Authority partner'))
                    ...
                    if len(due_list) == 0:
                        raise orm.except_orm(
                            ...
                            % invoice.company_id.
                            withholding_payment_term_id.name)

                    period_ids = priod_obj.find(
                        cr, uid, dt=voucher.date, context=context)
                    new_move = {
                        'journal_id':
                            invoice.company_id.withholding_journal_id.id,
                        ...
                        'line_id': [
                            ...
                            (0, 0, {
                                'name': _('Payable withholding - ') +
                                invoice.number,
                                'account_id':
                                    invoice.company_id.
                                    withholding_account_id.id,
                                'partner_id':
                                    invoice.company_id.authority_partner_id.id,
                                'debit': 0.0,
