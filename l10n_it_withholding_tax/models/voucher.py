                ...
                if 'move_line_id' in line and line['move_line_id']:
                    ...
                    if line['amount'] or amount_overflow_residual:
                        ...
                        if amount_overflow_residual:
                            if (line['amount'] + amount_overflow_residual) <= \
                ...
                for wt_v_line in v_line.withholding_tax_line_ids:
                    ...
                    for payment_line in payment_lines:
                        ...
                        val_move_line = {
                            ...
                            'name': wt_v_line.withholding_tax_id.name + ' ' 
                                + voucher.partner_id.name or '/',
                            'account_id': wt_account_id,
                            'move_id': move_id,
                            #'partner_id': voucher.partner_id.id,
                            'partner_id': False,
                            'currency_id': v_line.move_line_id.currency_id.id 
                                or False,
                            'analytic_account_id': v_line.account_analytic_id 
                                and v_line.account_analytic_id.id or False,
                            'quantity': 1,
                            'credit': p_credit,
                            'debit': p_debit,
                            'date': voucher.date,
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            ctx.update({
                'voucher_special_currency': 
                    line.voucher_id.payment_rate_currency_id 
                    and line.voucher_id.payment_rate_currency_id.id or False,
