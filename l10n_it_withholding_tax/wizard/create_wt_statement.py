...
class wizard_create_wt_statement(orm.TransientModel):
    ...
    _columns = {
        ...
        'withholding_tax_id': fields.many2one('withholding.tax', 
                                            'Withholding Tax', required=True),
        'wt_account_move_line_id': fields.many2one('account.move.line', 
                                        'Account Move line', required=True),
    ...
    def create_wt_statement(self, cr, uid, ids, context=None):
        ...
        for wiz in self.browse(cr, uid, ids):
            ...
            wt_move_id = self.pool['withholding.tax.move'].create(
                                                            cr, uid, val)
        
        view_ref = self.pool.get('ir.model.data').get_object_reference(cr, uid, 
