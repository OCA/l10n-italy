...
class WizardInvoiceManageAsset(models.TransientModel):
    ...
    def onchange_invoices(self):
        if self.invoice_ids:
            ...
            invoice_type = invoices[0].move_type

            if any([inv.move_type != invoice_type for inv in invoices]):
                invoice_type = 'wrong'
            ...
            if all([inv.state in ('posted', 'open', 'paid') for inv in invoices]):
    ...
    def check_pre_create_asset(self):
        ...
        if not len(self.invoice_line_ids.mapped('move_id')) == 1:
            raise ValidationError(
    ...
    def get_create_asset_vals(self):
        ...
        purchase_invoice = self.invoice_line_ids.mapped('move_id')
        return {
            ...
            'supplier_ref': purchase_invoice.ref or "",
            'used': self.used,
        }
    ...
    def get_dismiss_asset_vals(self):
        ...
        invoice = self.invoice_line_ids.mapped('move_id')
        inv_num = invoice.name
    ...
    def get_update_asset_vals(self):
        ...
        for l in self.invoice_line_ids:
            ...
            if inv not in grouped_invoice_lines:
                grouped_invoice_lines[inv] = self.env['account.move.line']
