...
class ReportAsset(models.TransientModel):
    ...
    def format_amount(self, amount, currency=None):
        ...
        return float_repr(amount, min(2, currency.decimal_places))
        #return format_monetary(amount, currency)
    ...
    def get_purchase_vals(self):
        ...
        elif asset.purchase_invoice_id.ref:
            purchase_vals['partner_ref'] = asset.purchase_invoice_id.ref
        ...
        if asset.purchase_invoice_id:
            purchase_vals.update({
                'document_nr': asset.purchase_invoice_id.name or "/",
    ...
    def get_sale_vals(self):
        ...
        if asset.sale_invoice_id:
            sale_vals.update({
                'document_nr': asset.sale_invoice_id.name or "/",
...
class ReportDepreciation(models.TransientModel):
    ...
    def format_amount(self, amount, currency=None):
        ...
        currency = currency or self.get_currency()
        return float_repr(amount, min(2, currency.decimal_places))
        #return format_monetary(self.env, amount, currency)
        ...
        for dep_line in self.dep_line_ids.filtered(
            ...
            for num, aa_info in enumerate(dep_line.asset_accounting_info_ids):
                ...
                if aa_info.invoice_line_id:
                    inv = aa_info.invoice_line_id.move_id
                    vals = {
                        'document_date': format_date(
                            dep_line, 'date', '%d-%m-%Y'
                        ),
                        'document_nr': inv.name or "/",
                        'partner_name': inv.partner_id.name or "/",
                        'partner_ref': inv.ref or "/",
