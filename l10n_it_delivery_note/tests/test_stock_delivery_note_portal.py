    ...
    def setUp(self):
        ...
        dn_form = Form(
            self.env["stock.delivery.note.create.wizard"].with_context(
                **{"active_id": self.picking_mr.id, "active_ids": self.picking_mr.ids}
            )
        )
        dn = dn_form.save()
        dn.confirm()
        ...
        self.picking_ab.move_ids[1].quantity = 1

        self.picking_ab.button_validate()
        dn_form = Form(
            self.env["stock.delivery.note.create.wizard"].with_context(
                **{"active_id": self.picking_ab.id, "active_ids": self.picking_ab.ids}
            )
        )
        dn = dn_form.save()
        dn.confirm()
        ...
        dn_form = Form(
            self.env["stock.delivery.note.create.wizard"].with_context(
                **{"active_id": self.picking_ab_mr.id, "active_ids": self.picking_ab_mr.ids}
            )
        )
        dn = dn_form.save()
        dn.confirm()
        ...
        dn_form = Form(
            self.env["stock.delivery.note.create.wizard"].with_context(
                **{"active_id": self.picking_azr.id, "active_ids": self.picking_azr.ids}
            )
        )
        dn = dn_form.save()
        dn.confirm()
