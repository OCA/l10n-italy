...
class fiscal_year_closing_lp_account_mapping(osv.osv):
    ...
    _columns = {
        ...
        # Accounts
        'source_account_id': fields.many2one('account.account',
                                             'Source account',
                                             required=True,
                                             ondelete='cascade'),
        'dest_account_id': fields.many2one('account.account',
                                           'Dest account',
                                           required=False,
                                           ondelete='cascade'),
    }
...
class fiscal_year_closing(osv.osv):
    ...
    _columns = {
        ...
        'loss_and_profit_move_id': fields.many2one('account.move',
                                                   'L&P Move',
                                                   ondelete='set null',
                                                   readonly=True),
        'create_net_loss_and_profit':
            fields.boolean('Create Net Loss & Profit'),
        'net_loss_and_profit_move_id': fields.many2one('account.move',
                                                       'Net L&P Move',
                                                       ondelete='set null',
                                                       readonly=True),
        'create_closing': fields.boolean('Close fiscal year'),
        'closing_move_id': fields.many2one('account.move',
                                           'Closing Move',
                                           ondelete='set null',
                                           readonly=True),
        'create_opening': fields.boolean('Open next fiscal year'),
        'opening_move_id': fields.many2one('account.move',
                                           'Opening Move',
                                           ondelete='set null',
                                           readonly=True),
        ...
        # Extra operations
        #
        'check_invalid_period_moves':
            fields.boolean('Check invalid period or date moves',
                           help="Checks that there are no moves,"
                                " on the fiscal year that is being closed,"
                                " with dates or periods outside that fiscal"
                                " year."),
        'check_draft_moves':
            fields.boolean('Check draft moves',
                           help="Checks that there are no draft moves on the"
                                " fiscal year that is being closed."
                                " Non-confirmed moves won't be taken in"
                                " account on the closing operations."),
        'check_unbalanced_moves':
            fields.boolean('Check unbalanced moves',
        ...
        'lp_account_mapping_ids':
            fields.one2many('account_fiscal_year_closing.fyc_lp_account_map',
                            'fyc_id',
                            'Account mappings'),
        ...
        'nlp_account_mapping_ids':
            fields.one2many('account_fiscal_year_closing.fyc_nlp_account_map',
                            'fyc_id',
                            'Account mappings'),
        ...
        'c_account_mapping_ids':
            fields.one2many('account_fiscal_year_closing.fyc_c_account_map',
                            'fyc_id',
                            'Accounts'),
    ...
    def _get_closing_fiscalyear_id(self, cr, uid, context):
        ...
        if not fiscalyear_ids:
            fiscalyear_ids = self.pool.get('account.fiscalyear').\
                search(cr,
                       uid,
                       [('company_id', '=', False),
                        ('date_start', '<=', str_date),
        ...
        for source, dest, description in mapping:
            ...
            if source_account_id:
                ...
                if dest_account_id:
                    ...
                    # Add the line to the result
                    account_mappings.append({
                        'name': _('No destination account %s'
                                  ' found for account %s.') % (dest, source),
                        'source_account_id': source_account_id,
    ...
    def action_draft(self, cr, uid, ids, context=None):
        ...
        for fyc in self.browse(cr, uid, ids, context):
            ...
            fyc_ids = self.search(
                cr, uid, [('closing_fiscalyear_id',
                           '=',
                           fyc.closing_fiscalyear_id.id)])
            if len(fyc_ids) > 1:
                raise osv.except_osv(_('Error'), _(
                    'There is already a fiscal year closing'
                    ' for the fiscal year to close.'))

            assert fyc.opening_fiscalyear_id and fyc.opening_fiscalyear_id.id
            fyc_ids = self.search(
                cr, uid, [('opening_fiscalyear_id',
                           '=',
                           fyc.opening_fiscalyear_id.id)])
            if len(fyc_ids) > 1:
                raise osv.except_osv(_('Error'), _(
                    'There is already a fiscal year closing'
                    ' for the fiscal year to open.'))

            #
            # Check whether the default values of the fyc object
            # have to be computed
            ...
            if fyc.c_account_mapping_ids:
                ...
                vals = {
                    ...
                    'lp_journal_id': self._get_journal_id(cr,
                                                          uid,
                                                          fyc,
                                                          context),
                    'lp_period_id': self._get_lp_period_id(cr,
                                                           uid,
                                                           fyc,
                                                           context),
                    'lp_date': fyc.closing_fiscalyear_id.date_stop,
                    #   'lp_account_mapping_ids':
                    #     self._get_account_mappings(cr, uid, fyc,
                    #        _LP_ACCOUNT_MAPPING, context),

                    #
                    # Net L&P options
                    #
                    'nlp_description': _("Net Loss & Profit"),
                    'nlp_journal_id': self._get_journal_id(cr,
                                                           uid,
                                                           fyc,
                                                           context),
                    'nlp_period_id': self._get_lp_period_id(cr,
                                                            uid,
                                                            fyc,
                                                            context),
                    ...
                    #   'nlp_account_mapping_ids':
                    #     self._get_account_mappings(cr, uid, fyc,
                    #       _NLP_ACCOUNT_MAPPING, context),

                    #
                    # Closing options
                    #
                    'c_description': _("Fiscal Year Closing"),
                    'c_journal_id': self._get_journal_id(cr,
                                                         uid,
                                                         fyc,
                                                         context),
                    'c_period_id': self._get_c_period_id(cr,
                                                         uid,
                                                         fyc,
                                                         context),
                    ...
                    #    'c_account_mapping_ids':
                    #       self._get_account_mappings(cr, uid, fyc,
                    #          _C_ACCOUNT_MAPPING, context),

                    #
                    # Opening options
                    #
                    'o_description': _("Fiscal Year Opening"),
                    'o_journal_id': self._get_journal_id(cr,
                                                         uid,
                                                         fyc,
                                                         context),
                    'o_period_id': self._get_o_period_id(cr,
                                                         uid,
                                                         fyc,
                                                         context),
