...
class wizard_run(osv.osv_memory):
    ...
    def run(self, cr, uid, ids, context=None):
        ...
        #
        if fyc.create_net_loss_and_profit and \
        ...
        #
        if (not fyc.create_net_loss_and_profit) and \
                fyc.net_loss_and_profit_move_id:
        ...
        #
        if not fyc.create_opening and \
                not fyc.create_closing and \
    ...
    def _check_invalid_period_moves(self, cr, uid, fyc, context):
        """
        Checks for moves with invalid period
        ...
        if len(account_move_ids):
            ...
            str_invalid_period_moves = '\n'.join(
                ['id: %s, date: %s, number: %s, ref: %s' % (
                 ...
                 move.date,
                 move.name,
                 move.ref) for move in invalid_period_moves])
            raise osv.except_osv(_('Error'),
                                 _(
                'One or more moves with invalid period'
                ' or date found on the fiscal year: \n%s') %
                str_invalid_period_moves)
        ...
        if len(account_move_ids):
            ...
            str_draft_moves = '\n'.join(
                ['id: %s, date: %s, number: %s, ref: %s' % (
                 ...
                 move.date,
                 move.name,
                 move.ref) for move in draft_moves])
    ...
    def _check_unbalanced_moves(self, cr, uid, fyc, context):
        ...
        #
        unbalanced_moves = []
        for move in pool.get('account.move').browse(cr,
                                                    uid,
                                                    account_move_ids,
                                                    context):
            amount = 0
        ...
        if len(unbalanced_moves):
            str_unbalanced_moves = '\n'.join(
                ['id: %s, date: %s, number: %s, ref: %s' % (
                 ...
                 move.date,
                 move.name,
                 move.ref) for move in unbalanced_moves])
            raise osv.except_osv(_('Error'),
                                 _(
    ...
    def create_closing_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'loss_and_profit':
            ...
            for account_map in account_mapping_ids:
                if not account_map.dest_account_id:
                    raise osv.except_osv(_('UserError'),
                                         _(
                        "The L&P account mappings"
                        " are not properly configured: %s") % account_map.name)
            ...
            assert operation in ('loss_and_profit',
                                 'net_loss_and_profit',
                                 'close'), \
        ...
        for account_map in account_mapping_ids:
            ...
            if account_map.dest_account_id:
                dest_accounts_totals[
                    account_map.dest_account_id.id] = dest_accounts_totals.get(
            ...
            for account in pool.get('account.account').browse(cr,
                                                              uid,
                                                              child_ids,
                                                              ctx):
    ...
    def create_opening_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'open':
            ...
            if not (closing_move and closing_move.id):
                raise osv.except_osv(_('UserError'), _(
                    "The closing move must exist to create the opening one"))
