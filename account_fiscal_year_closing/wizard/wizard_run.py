...
class wizard_run(osv.osv_memory):
    ...
    def run(self, cr, uid, ids, context=None):
        ...
        if fyc.create_net_loss_and_profit\
            ...
            self.create_closing_move(cr, uid, 'net_loss_and_profit', fyc,
                                     context)
        #
        # Remove the Net L&P move if needed
        #
        if (not fyc.create_net_loss_and_profit)\
        ...
        #
        if not fyc.create_opening and not fyc.create_closing\
    ...
    def _check_invalid_period_moves(self, cr, uid, fyc, context):
        ...
        if len(account_move_ids):
            ...
            raise osv.except_osv(
                _('Error'), _('One or more moves with invalid period or date\
                 found on the fiscal year: \n%s') % str_invalid_period_moves)
    ...
    def create_closing_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'loss_and_profit':
            ...
            for account_map in account_mapping_ids:
                if not account_map.dest_account_id:
                    raise osv.except_osv(
                        _('UserError'), _("The L&P account mappings are not\
                         properly configured: %s") % account_map.name)
            ...
            if not fyc.nlp_description:
                raise osv.except_osv(
                    _('UserError'), _("The Net L&P description must be\
                     defined"))
            if not fyc.nlp_date:
                raise osv.except_osv(_('UserError'),
            ...
            if not fyc.c_description:
                raise osv.except_osv(_('UserError'),
                                     _("The closing description must be\
                                      defined"))
            if not fyc.c_date:
                raise osv.except_osv(_('UserError'),
        ...
        for account_map in account_mapping_ids:
            ...
            for account in pool.get('account.account').browse(
                ...
                #     (account.user_type.close_method) and always do a balance
                #     close.
                if account.type != 'view':
                    ...
                    if round(abs(balance),
                             pool.get('decimal.precision').precision_get(
                            cr, uid, 'Account')) > 0:
                        #
    ...
    def create_opening_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'open':
            ...
            if not (closing_move and closing_move.id):
                raise osv.except_osv(
                    _('UserError'), _("The closing move must exist to create\
                    the opening one"))
            if not closing_move.line_id:
                raise osv.except_osv(_('UserError'), _("The closing move \
                shouldn't be empty"))
            #
            # Get the values for the lines
            #
            if not fyc.o_description:
                raise osv.except_osv(_('UserError'),
                                     _("The opening description must be \
                                     defined"))
            if not fyc.o_date:
                raise osv.except_osv(_('UserError'),
