...
class WizardRun(osv.osv_memory):
    """
    ...
    def run(self, cr, uid, ids, context=None):
        ...
        #
        if not fyc.create_opening and not fyc.create_closing and \
                not not fyc.create_net_loss_and_profit \
    ...
    def _check_invalid_period_moves(self, cr, uid, fyc, context):
        ...
        account_move_ids = pool.get('account.move').search(
            cr, uid, [
        ...
        account_move_ids.extend(pool.get('account.move').search(
            cr, uid, [
                ('period_id', 'in', period_ids),
        ...
        if len(account_move_ids):
            ...
            raise osv.except_osv(
                _('Error'),
                _('One or more moves with invalid period or date found '
    ...
    def _check_draft_moves(self, cr, uid, fyc, context):
        ...
        if len(account_move_ids):
            draft_moves = pool.get('account.move').browse(
                cr, uid, account_move_ids, context)
            str_draft_moves = '\n'.join(['id: %s, date: %s, number: %s, ref: %s' %
                                         (move.id, move.date, move.name, move.ref)
    ...
    def _check_unbalanced_moves(self, cr, uid, fyc, context):
        ...
        for move in pool.get('account.move').browse(cr, uid, account_move_ids, context):
            ...
            if round(abs(amount), dp.precision_get(cr, uid, 'Account')) > 0:
    ...
    def create_closing_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'loss_and_profit':
            ...
            for account_map in account_mapping_ids:
                if not account_map.dest_account_id:
                    raise osv.except_osv(
                        _('UserError'),
                        _("The L&P account mappings are not properly configured: %s")
                        % account_map.name)
            ...
            if not fyc.lp_description:
                raise osv.except_osv(_('UserError'),
                                     _("The L&P description must be defined"))
            if not fyc.lp_date:
                raise osv.except_osv(_('UserError'),
                                     _("The L&P date must be defined"))
            if not (fyc.lp_period_id and fyc.lp_period_id.id):
                raise osv.except_osv(_('UserError'),
                                     _("The L&P period must be defined"))
            if not (fyc.lp_journal_id and fyc.lp_journal_id.id):
                raise osv.except_osv(_('UserError'),
                                     _("The L&P journal must be defined"))
            ...
            for account_map in account_mapping_ids:
                if not account_map.dest_account_id:
                    raise osv.except_osv(_('UserError'),
                    _("The Net L&P account mappings are not properly configured: %s")
                    % account_map.name)
            '''
            #
            # Get the values for the lines
            ...
            if not fyc.nlp_date:
                raise osv.except_osv(_('UserError'),
                                     _("The Net L&P date must be defined"))
            if not (fyc.nlp_period_id and fyc.nlp_period_id.id):
                raise osv.except_osv(_('UserError'),
                                     _("The Net L&P period must be defined"))
            if not (fyc.nlp_journal_id and fyc.nlp_journal_id.id):
                raise osv.except_osv(_('UserError'),
                                     _("The Net L&P journal must be defined"))
            ...
            if not (fyc.loss_and_profit_move_id and fyc.loss_and_profit_move_id.id):
                raise osv.except_osv(
                    _('UserError'),
                    _("The L&P move must exist before creating the closing one"))
            '''
            #
            ...
            if not fyc.c_date:
                raise osv.except_osv(_('UserError'),
                                     _("The closing date must be defined"))
            if not (fyc.c_period_id and fyc.c_period_id.id):
                raise osv.except_osv(_('UserError'),
                                     _("The closing period must be defined"))
            if not (fyc.c_journal_id and fyc.c_journal_id.id):
                raise osv.except_osv(_('UserError'),
                                     _("The closing journal must be defined"))
        ...
        accounts_done = 0
        for account_map in account_mapping_ids:
            # Init (if needed) the dictionary that will store the
            # totals for the dest accounts
            if account_map.dest_account_id:
                dest_accounts_totals[account_map.dest_account_id.id] = \
                    dest_accounts_totals.get(account_map.dest_account_id.id, 0)
            ...
            # For each children account. (Notice the context filter!
            # the computed balanced is based on this filter)
            for account in pool.get('account.account').browse(cr, uid, child_ids, ctx):
                ...
                if account.type != 'view':
                    # Compute the balance for the account
                    # (uses the previous browse context filter)
                    balance = account.balance
                    # Check if the balance is greater than the limit
                    if round(abs(balance), dp.precision_get(cr, uid, 'Account')) > 0:
                        #
                        ...
                        # Update the dest account total
                        # (with the inverse of the balance)
                        if account_map.dest_account_id:
                            dest_accounts_totals[account_map.dest_account_id.id] \
                                -= balance
            accounts_done += 1
    ...
    def create_opening_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'open':
            ...
            if not fyc.o_date:
                raise osv.except_osv(
                    _('UserError'),
                    _("The opening date must be defined"))
            if not (fyc.o_period_id and fyc.o_period_id.id):
                raise osv.except_osv(
                    _('UserError'),
                    _("The opening period must be defined"))
            if not (fyc.o_journal_id and fyc.o_journal_id.id):
                raise osv.except_osv(
                    _('UserError'),
                    _("The opening journal must be defined"))
