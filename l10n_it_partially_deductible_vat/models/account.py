...
class AccountTax(models.Model):
    ...
    def compute_all(self, cr, uid, taxes, price_unit, quantity, product=None, partner=None, force_excluded=False):
        '''
        Modify compute_all function in order to put correct total in case of tax with one child (usually partially deductible)'''

        res = super(AccountTax, self).compute_all(cr, uid, taxes, price_unit, quantity, product=product, partner=partner, force_excluded=force_excluded)
        precision = 2  # è sempre 2 per quanto viene addebitato, anche se impostato diversamente in 'Account' # self.pool['decimal.precision'].precision_get(cr, uid, 'Account')
        tax_list = res['taxes']
        totalex = res['total']
        if len(tax_list) == 2:
            for tax in tax_list:
                if tax.get('balance', False):
                    ...
                    ind_tax['price_unit'] = round(base_ind / quantity, self.pool['decimal.precision'].precision_get(cr, uid, 'Product Price'))
                    tax['price_unit'] = round(base_ded / quantity, self.pool['decimal.precision'].precision_get(cr, uid, 'Product Price'))
        return res
            ...
            if not inv_tax['base_code_id'] and inv_tax['tax_code_id']:
                ...
                for inv_tax_2 in tax_grouped.values():
                    ...
                    if inv_tax_2['base_code_id'] and not inv_tax_2['tax_code_id']:
                        main_tax = tax_obj.get_main_tax(tax_obj.get_account_tax_by_base_code(
                            tax_code_obj.browse(inv_tax_2['base_code_id'])))
                        ...
                        if main_tax.id == tax.id:
                            ...
                            inv_tax['tax_amount'] = cur.round(inv_tax['tax_amount'])
                            inv_tax_2['amount'] = cur.round(inv_tax_2['amount'])
        return tax_grouped
