...
class WebsiteSaleFatturapa(WebsiteSale):
        ...
        if all(f in values for f in ["name", "firstname", "lastname"]):
            ...
            pre_values.update(
                name=partner_model._get_computed_name(
                    pre_values["lastname"], pre_values["firstname"]
                )
            )
        return pre_values
    ...
    def checkout_form_validate(self, mode, all_form_values, data):
        ...
        check_fatturapa_fields = partner_model._check_ftpa_partner_data._constrains
    ...
    def _checkout_form_save(self, mode, checkout, all_values):
        for fatturapa_field in self._get_fatturapa_fields():
            if fatturapa_field not in checkout and fatturapa_field in all_values:
