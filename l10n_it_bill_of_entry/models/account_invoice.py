    ...
    def action_move_create(self):
        ...
        for invoice in self:
            if invoice.customs_doc_type == "forwarder_invoice":
                ...
                boe_amounts = invoice.forwarder_bill_of_entry_ids.mapped("amount_total")
                ...
                for move_line in move.line_ids:
                    for boe in invoice.forwarder_bill_of_entry_ids:
                        if move_line.account_id.id == boe.account_id.id:
                            reconcile_ids.append(move_line.id)
                            for boe_move_line in boe.move_id.line_ids:
                                if boe_move_line.account_id.id == boe.account_id.id:
