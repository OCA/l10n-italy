    ...
    _columns = {
        ...
        'supplier_invoice_ids':
            fields.many2many('account.invoice',
                             ...
                             'sboe_id',
                             'Supplier Invoices'),
        'forwarder_invoice_id': fields.many2one('account.invoice',
                                                'Forwarder Invoice'),
        'forwarder_bill_of_entry_ids':
            fields.one2many('account.invoice',
                            ...
                            readonly=True),
        'bill_of_entry_storno_id': fields.many2one('account.move',
                                                   'Bill of Entry Storno',
                                                   readonly=True),
    }
        ...
        for invoice in self.browse(cr, uid, ids, context):
            if invoice.customs_doc_type == 'forwarder_invoice':
                ...
                if not advance_customs_vat_line:
                    raise orm.except_orm(
                        _('Error'),
                        _("Forwarder invoice %s does not have lines "
                    ...
                    for boe_line in bill_of_entry.invoice_line:
                        ...
                        if boe_line.invoice_line_tax_id:
                            if len(boe_line.invoice_line_tax_id) > 1:
                                raise orm.except_orm(
                                    _('Error'),
                                    _("Can't handle more than 1 tax"
                                      " for line %s") % boe_line.name)
                            tax_code_id = (boe_line.invoice_line_tax_id[0].
                                           base_code_id and
                ...
                reconcile_ids = []
                for move_line in move_obj.browse(cr,
                                                 uid,
                                                 move_id,
                                                 context).line_id:
                    for bill_of_entry in invoice.forwarder_bill_of_entry_ids:
                        if move_line.account_id.id == bill_of_entry.\
                                account_id.id:
                            reconcile_ids.append(move_line.id)
                            for boe_move_line in bill_of_entry.move_id.\
                                    line_id:
                                if boe_move_line.account_id.id == \
        ...
        for invoice in self.browse(cr, uid, ids, context):
            if invoice.bill_of_entry_storno_id:
                account_move_obj.button_cancel(
                    cr,
                    uid,
                    [invoice.bill_of_entry_storno_id.id],
                    context=context)
                account_move_obj.unlink(cr,
                                        uid,
                                        [invoice.bill_of_entry_storno_id.id],
                                        context=context)
        return res
