...
class riba_file_export(osv.osv_memory):
    ...
    def act_getfile(self, cr, uid, ids, context=None):
        ...
        order_obj = self.pool.get('riba.order').browse(cr, uid, active_ids, context=context)[0]
        credit_bank = order_obj.mode.bank_id
        name_company = order_obj.company_id.partner_id.name
        ...
        if not order_obj.company_id.partner_id.vat and not order_obj.company_id.partner_id.fiscalcode:
        ...
        for line in order_obj.line_ids:
            ...
            if not line.date:
                ...
                due_date = datetime.datetime.strptime(line.date[:10], '%Y-%m-%d').strftime("%d%m%y")
            ...
            Riba = [
                        ...
                        line.ml_inv_ref.internal_number,
                        datetime.datetime.strptime(line.ml_date_created, '%Y-%m-%d').strftime("%d/%m/%Y"),
                        ]
