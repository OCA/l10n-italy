...
def _extract_balance_report_columns(columns, mode):
    return {
        col_index: col_values
        for col_index, col_values in columns.items()
        if mode in col_values["balance_report_mode"]
    }
...
class AccountBalanceReportXslx(models.AbstractModel):
    _name = "report.l10n_it_account_balance_report.report_xlsx"
    _description = "Account Balance XLSX Report"
    ...
    def _define_formats(self, workbook, report_data):
        ...
        report_data["balance_report_currency"] = currency
    ...
    def _get_report_name(self, report, data=False):
        ...
        * Overrides standard method *
        ...
        """
        rep_type = data.get("account_balance_report_type")
        cols = (
            self.env["report.l10n_it_account_balance_report.account_balance_report"]
            .get_column_data()
            .get(rep_type)
        )
        title = cols["title"]
    ...
    def generate_section_cols(self, cols, mode):
        ...
        for col_values in section_cols.values():
            col_values["balance_report_mode"] = ("section", mode)
        return section_cols
    ...
    def generate_partner_columns(self, cols, mode, columns):
        ...
        for col_index, col_values in enumerate(partner_columns):
            col_values["balance_report_mode"] = ("partner", mode)
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        ...
        report_result = self.env[
            "report.l10n_it_account_balance_report.account_balance_report"
        ...
        report_data["balance_report_cols"] = cols
    ...
    def write_main_headers(self, report_data):
        ...
        left_columns = _extract_balance_report_columns(report_data["columns"], "left")
        report_data["sheet"].merge_range(
            report_data["row_pos"],
            min(left_columns.keys()),
            report_data["row_pos"],
            max(left_columns.keys()),
            report_data["balance_report_cols"]["left"]["name"],
            report_data["formats"]["format_header_center"],
        )
        right_columns = _extract_balance_report_columns(report_data["columns"], "right")
        report_data["sheet"].merge_range(
            report_data["row_pos"],
            min(right_columns.keys()),
            report_data["row_pos"],
            max(right_columns.keys()),
            report_data["balance_report_cols"]["right"]["name"],
            report_data["formats"]["format_header_center"],
        )
        report_data["row_pos"] += 1
    ...
    def generate_table(self, report, report_data, report_result):
        ...
        report_data["balance_report_left_lines"] = left_lines
        right_lines = self.get_report_lines(
            "section_credit_ids", report, report_data, report_result
        )
        report_data["balance_report_right_lines"] = right_lines
        table = self.get_table_data(report_data)

        balance_report_cols = report_data["balance_report_cols"]
    ...
    def get_table_data(self, report_data):
        table = {}
        left_lines = report_data["balance_report_left_lines"]
        right_lines = report_data["balance_report_right_lines"]
    ...
    def get_line_info(self, report, report_data, report_result, row=None):
        ...
        if l:
            cols_dict = _extract_balance_report_columns(report_data["columns"], "left")
            for c in cols_dict:
        ...
        if r:
            cols_dict = _extract_balance_report_columns(report_data["columns"], "right")
            for c in cols_dict:
                value, style, allow = self.get_write_data(
    ...
    def get_write_data(self, line, col_dict, report, report_data, report_result):
        ...
        currency = report_data["balance_report_currency"]
    ...
    def write_total_balance(self, report, data, report_data, report_result):
        ...
        if surplus or deficit:
            ...
            left_columns = _extract_balance_report_columns(
            ...
            right_columns = _extract_balance_report_columns(
