...
class AccountBalanceReportXslx(models.AbstractModel):
    ...
    def _define_formats(self, workbook):
        ...
        currency = self.report.company_id.currency_id \
    ...
    def _get_report_filters(self, report):
        ...
        * Overrides standard method *.
        ...
        """
        date_from = format_date(report.date_from)
        date_to = format_date(report.date_to)
        return [
            (_("Date range filter"),
    ...
    def get_write_data(self, line, col_dict):
        ...
        if cell_type == 'many2one':
            ...
            if currency:
                ...
                value = line.currency_id.round(float(value))
                if getattr(line, 'account_group_id', False):
                    style = self.format_amount_bold_right
                else:
                    style = self.format_amount_right
    ...
    def write_sections_balance(self):
        ...
        report = self.report
        curr = report.company_id.currency_id or report.company_id._get_euro()
        credit_data = order_currency_amount(curr, report.total_credit)
        debit_data = order_currency_amount(curr, report.total_debit)
    ...
    def write_total_balance(self):
        ...
        report = self.report
        curr = report.company_id.currency_id or report.company_id._get_euro()
        balance = '%.2f' % report.total_balance
        total_credit = report.total_credit
        total_debit = report.total_debit
        digits = curr.decimal_places
        if not digits:
            digits = self.env['decimal.precision'].precision_get('Account')
        surplus = float_compare(total_credit, total_debit, digits) == 1
        deficit = float_compare(total_credit, total_debit, digits) == -1
