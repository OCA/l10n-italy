...
class DDT(models.Model):
    ...
    virtual_mass = fields.Html(string="Virtual Mass", compute="_compute_virtual_mass")
    is_adr = fields.Boolean(string="Is Adr", compute="_compute_is_adr")
    ...
    def _compute_virtual_mass(self):
        ...
        limit = (
            self.env["ir.config_parameter"].sudo().get_param("adr_virtual_mass_limit")
        )
        min_limit = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("adr_virtual_mass_min_limit")
        )
        msg_exceeded = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("adr_limit_exceeded_message")
        )
        msg_not_exceeded = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("adr_limit_not_exceeded_message")
        )
            ...
            for record in self:
                ...
                for line in record.line_ids:
                    ...
                    if product.adr_category_id and qty > float(min_limit):
                        total += qty * product.adr_category_id.multiplier
                ...
                if float(total) <= float(limit):
                    msg += "<br/>%s" % msg_not_exceeded.format(virtual_mass=str(total))
                else:
                    msg += "<br/>%s" % msg_exceeded.format(virtual_mass=str(total))
