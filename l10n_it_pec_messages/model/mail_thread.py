...
class mail_thread(osv.AbstractModel):
    ...
    def _message_find_partners(self, cr, uid, message, header_fields=['From'], context=None):
        ...
        if not self.is_server_pec(cr,uid,context=context):
            ...
            for email_address in tools.email_split(s):
                related_partners = partner_obj.search(cr, uid, [('pec_mail', '=ilike', email_address), ('user_ids', '!=', False)], limit=1, context=context)
                if not related_partners:
                    related_partners = partner_obj.search(cr, uid, [('pec_mail', '=ilike', email_address)], limit=1, context=context)
                partner_ids += related_partners
            return partner_ids

    #need to write new method  payload beacuse the original method
    #not heve context .
    def _message_extract_payload_pec(self, message, save_original=False,context=None):
    ...
    def message_parse(self, cr, uid, message, save_original=False, context=None):
        ...
        if message.get('Reply-To'):
            author_ids = self._message_find_partners(cr, uid, message, ['Return-Path'], context=context)
            if author_ids:
