...
from openerp import _, api, fields, models, workflow
...
class BankingExportSepaCbiEsteroWizard(models.TransientModel):
    ...
    nb_transactions = fields.Integer(string="Number of Transactions", readonly=True)
    total_amount = fields.Float(string="Total Amount", readonly=True)
    file = fields.Binary(string="File", readonly=True)
    filename = fields.Char(string="Filename", readonly=True)
    payment_order_ids = fields.Many2many(
        "payment.order",
        "wiz_sepa_cbi_estero_payorders_rel",
        "wizard_id",
        "payment_order_id",
    ...
    def generate_party_agent(
        ...
        # ABI and BIC code
        if party_type == "Dbtr":
            company_bank = gen_args["sepa_export"].payment_order_ids[0].mode.bank_id
            partner_debitor = (
            ...
            debitor_country_node = etree.SubElement(debitor_address_node, "Ctry")
            debitor_country_node.text = iban[:2]
            debitor_address_line_node = etree.SubElement(
                debitor_address_node, "AdrLine"
            )
            if partner_debitor:
                address = "%s %s %s" % (
                    partner_debitor.street or "",
                    partner_debitor.city or "",
                    partner_debitor.country_id
                    and partner_debitor.country_id.name
                    or "",
                )
    ...
    def create_sepa(self):
        ...
        convert_to_ascii = self.payment_order_ids[0].mode.convert_to_ascii
        ...
        pain_03_to_05 = ["CBIBdyCrossBorderPaymentRequest.00.01.01"]
        # A. Group header
        (
            group_header_1_0,
        ...
        for (requested_date, priority), lines in lines_per_group.items():
            ...
            for line in lines:
                ...
                if not line.bank_id:
                    raise UserError(
                        _(
                            "Missing Bank Account on invoice '%s' (payment "
                            "order line reference '%s')"
                        )
                        % (line.ml_inv_ref.number, line.name)
                    )
                self.generate_party_block(
                    credit_transfer_transaction_info_2_27,
                    "Cdtr",
                    "C",
                ...
                creditor_node = credit_transfer_transaction_info_2_27.xpath("//Cdtr")[
                    transactions_count_1_6 - 1
                ]
                ...
                if not iso_country:
                    raise UserError(
                        _(
                            "Missing Country for Partner '%s' (payment "
                            "order line reference '%s')"
                        )
                ...
                if partner_creditor:
                    address = "%s %s %s" % (
                        ...
                        partner_creditor.country_id
                        and partner_creditor.country_id.name
                        or "",
                    )
