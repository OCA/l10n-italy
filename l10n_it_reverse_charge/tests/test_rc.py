...
class TestReverseCharge(ReverseChargeCommon):
    ...
    def test_intra_EU(self):
        ...
        invoice = self.create_invoice(self.supplier_intraEU, amounts=[100], taxes=self.tax_22ai)
        ...
        self.assertEqual(
            invoice.rc_self_invoice_id.payment_id.move_id.state, "posted"
        )
    ...
    def test_intra_EU_2_mixed_lines(self):
        ...
        invoice = self.create_invoice(self.supplier_intraEU, amounts=[100,200], taxes=self.tax_22ai, post=False)
    ...
    def test_intra_EU_amount_tax_amount_payments_widget_discrepancy(self):
        ...
        invoice = self.create_invoice(self.supplier_intraEU, amounts=inv_amounts, taxes=self.tax_22ai)
    ...
    def test_extra_EU(self):
        invoice = self.create_invoice(self.supplier_extraEU, amounts=[100], taxes=self.tax_22ai)
    ...
    def test_intra_EU_zero_total(self):
        invoice = self.create_invoice(self.supplier_intraEU, [100, -100], taxes=self.tax_22ai)
    ...
    def test_new_refund_flag(self):
        ...
        invoice = self.create_invoice(self.supplier_extraEU, [100], taxes=self.tax_22ai, post=False)
        self.assertTrue(all(line.rc for line in invoice.invoice_line_ids))

    def test_intra_EU_exempt(self):
        invoice = self.create_invoice(self.supplier_extraEU, [100], taxes=self.tax_22ai)
