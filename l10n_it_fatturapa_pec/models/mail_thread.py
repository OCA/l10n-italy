...
class MailThread(orm.AbstractModel):
    ...
    def message_route(self, cr, uid, message, model=None, thread_id=None, custom_values=None, context=None):
        if any(FATTURAPA_ACCOUNT in x for x in [
            ...
            _logger.info("Processing FatturaPA PEC with Message-Id: {}".format(message.get('Message-Id')))
            ...
            fatturapa_attachments = [a for a in message_dict['attachments'] if fatturapa_regex.match(a[0])]
            response_regex = re.compile(RESPONSE_MAIL_REGEX)
            response_attachments = [a for a in message_dict['attachments'] if response_regex.match(a[0])]
        ...
        return super(MailThread, self).message_route(cr, uid, message, model=model, thread_id=thread_id,
    ...
    def find_attachment_by_subject(self, cr, uid, subject, context=None):
        if 'CONSEGNA: ' in subject:
            ...
            fatturapa_attachment_out_id = self.pool.get('fatturapa.attachment.out').search(cr, uid,
                [('datas_fname', '=', attachment_name)], context=context)
    ...
    def create_fatturapa_attachment_in(self, cr, uid, attachment, context=None):
            ...
            with zipfile.ZipFile(io.BytesIO(decoded)) as zf:
                for file_name in zf.namelist():
                    ...
                    if fatturapa_regex.match(file_name):
                        ...
                        fatturapa_attachment_ids = fatturapa_attachment_in.search(cr, uid, [('name', '=', file_name)],
                            context=context)
                        if fatturapa_attachment_ids:
                            _logger.info("In invoice %s already processed" % file_name)
                        else:
