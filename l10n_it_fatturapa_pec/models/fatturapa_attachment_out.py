...
class FatturaPAAttachmentOut(orm.Model):
    ...
    def send_via_pec(self, cr, uid, ids, context=None):
        ...
        for attachment in attachments:
            mail_message_id = self.pool['mail.message'].create(cr, uid, {
                'model': self._name,
                'res_id': attachment.id,
                'subject': attachment.name,
                'body_txt': 'XML file for FatturaPA {} sent to Exchange System to '
                        'the email address {}.'
                .format(
                    attachment.name,
            ...
            #    'mail_server_id': company.sdi_channel_id.pec_server_id.id,
            #})
            mail_id = mail_message_id
            mail_obj = mail_message_id
