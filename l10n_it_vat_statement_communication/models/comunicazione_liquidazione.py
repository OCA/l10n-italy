...
class ComunicazioneLiquidazione(models.Model):
    ...
    def _export_xml_get_intestazione(self):
        ...
        if not self.company_id.vsc_supply_code:
            raise ValidationError(
                _("Impostare codice fornitura nella configurazione \
                    della liquidazione IVA dell'azienda"))
        code = self.company_id.vsc_supply_code
        x1_1_1_CodiceFornitura.text = '{}'.format(code)
    ...
    def _export_xml_get_frontespizio(self):
        ...
        if self.date_commitment:
            ...
            x1_2_1_13_DataImpegno.text = date_commitment.strftime(
                '%d%m%Y')
        # FirmaIntermediario
        if self.delegate_fiscalcode:
